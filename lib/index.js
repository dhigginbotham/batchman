...
var Batchman = function (opts) {
  ...
  // path for your endpoint
  this.path = 'batchman';

  // options passed through for rate limiting
  this.delay = '35ms';
  this.after = 1;
  ...
  var processRequests = function (reqs, fn) {
    ...
    reqs.forEach(function (request, i) {
      ...
      sloth.limit(function () {
        ...
        self.request(request, function (err, body) {
  ...
  this.mount = function (app, fn) {
    ...
    var self = this,
        path = '/' + self.path,
        out = 'Successfully mounted batchman to `' + path + '`';
   
    // assign our route
    app.post(path, self.middleware, self.batchRequests, self.filter, self.end);
